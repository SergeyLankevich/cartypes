...
class Truck(CarBase):
    ...
    def __init__(self, brand, photo_file_name, carrying, body_whl):
        super().__init__(brand, photo_file_name, carrying)
        try:
            length, width, height = (float(t) for t in body_whl.split('x', 2))
        except ValueError:
...
def get_car_list(file):
    with open(file) as f_in:
        ...
        strats = {car_class.car_type: car_class for car_class in (Car, Truck, SpecMachine)}
